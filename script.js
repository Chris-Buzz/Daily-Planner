...
function renderTasks(day) {
  ...
  tasks[weekKey].sort((a, b) => {
    if (a.timestamp && b.timestamp) {
      // Compare timestamps if both tasks have them
      const timeA = a.timestamp;
      const timeB = b.timestamp;

      const convertTo24Hour = (time) => {
        let [hour, minute, period] = time.match(/(\d+):(\d+) ([AP]M)/).slice(1);
        hour = parseInt(hour);
      ...
      const minutesA = convertTo24Hour(timeA);
      const minutesB = convertTo24Hour(timeB);

      return minutesA - minutesB; // Compare in minutes
    } else if (a.timestamp) {
